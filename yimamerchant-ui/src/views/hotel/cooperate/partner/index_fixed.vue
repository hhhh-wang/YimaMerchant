  ...
  <div class="app-container">
    <el-form ref="queryForm" :model="queryParams" size="small" :inline="true">
      ...
      <el-form-item label="合作状态">
        <el-select v-model="queryParams.cooperateStatus" clearable placeholder="请选择合作状态">
    ...
    <el-row :gutter="16" class="mb16">
      <el-col :span="8">
        <el-card shadow="hover">
          <div slot="header">总合作酒店数</div>
          <div class="stat-num">{{ stats.totalCount || 0 }}</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover">
          <div slot="header">正常合作数</div>
          <div class="stat-num">{{ stats.normalCount || 0 }}</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover">
          <div slot="header">本月新增数</div>
          <div class="stat-num">{{ stats.monthlyNewCount || 0 }}</div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="签约时间" prop="signDate" width="160">
...
import {
  ...
  updatePartnerStatus
} from "@/api/hotel/cooperate/partner"
...
export default {
  ...
  methods: {
    ...
    getStats() {
      getPartnerStatistics(this.queryParams).then(res => {
        this.stats = res.data || {}
      })
    },
    handleQuery() {
    ...
    goDetail(row) {
      this.$router.push("/hotel/cooperate/partner-detail/" + row.hotelId)
    },
    changeStatus(row) {
      const nextStatus = row.cooperateStatus === "NORMAL" ? "PAUSED" : "NORMAL"
      this.$prompt("请输入处理原因", "调整合作状态", {
        inputValue: "业务调整"
      }).then(({ value }) => {
        updatePartnerStatus({
          hotelId: row.hotelId,
          cooperateStatus: nextStatus,
          operateReason: value
        }).then(() => {
    ...
    resetPwd(row) {
      resetPartnerPassword(row.hotelId).then(() => {
