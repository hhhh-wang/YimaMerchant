    ...
    <el-card v-loading="loading" style="margin-top: 16px;">
      <el-tabs>
        ...
        <el-tab-pane label="合同管理">
          <el-table :data="form.contracts || []">
            <el-table-column label="合同编号" prop="contractNo" />
            <el-table-column label="合同名称" prop="contractName" />
            <el-table-column label="开始日期" prop="contractStartDate" />
            <el-table-column label="结束日期" prop="contractEndDate" />
          </el-table>
        </el-tab-pane>
        <el-tab-pane label="账号管理">
          <el-table :data="form.accounts || []">
            <el-table-column label="账号名" prop="accountName" />
...
export default {
  ...
  methods: {
    getInfo() {
      this.loading = true
      getPartner(this.$route.params.hotelId)
        .then(res => {
          this.form = res.data || {}
        })
        .finally(() => {
